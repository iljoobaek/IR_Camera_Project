...
def read_data_and_evaluate(input_config, eval_config):
  ...
  if input_config.WhichOneof('input_reader') == 'tf_record_input_reader':
    ...
    for input_path in _generate_filenames(input_paths):
      ...
      if os.path.exists(FLAGS.result_path): os.remove(FLAGS.result_path)
      ...
      for string_record in record_iterator:
        ...
        saveboxes = decoded_dict["detection_boxes"]
        saveclasses = decoded_dict["detection_classes"]
        savescores = decoded_dict["detection_scores"]
        # threshold = savescores >= 0.5
        # saveboxes_ = saveboxes[threshold]
        # saveclasses_ = saveclasses[threshold]
        savename = decoded_dict["key"]
        savename_ = savename.split("/")[-1].split(".")[0]
        with open(FLAGS.result_path, 'a') as csvfile:
          for i in range(len(saveclasses)):
            metrics_writer = csv.writer(csvfile, delimiter=',')
            metrics_writer.writerow([savename_, saveclasses[i], savescores[i], saveboxes[i][1], saveboxes[i][0], saveboxes[i][3], saveboxes[i][2]])
...
def main(argv):
  del argv
  required_flags = ['input_config_path', 'eval_config_path', 'eval_dir', 'result_path']
