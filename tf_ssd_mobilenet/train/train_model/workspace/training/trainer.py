...
def tf_summary_image(images, bboxes,name='image'):
    """Add image with bounding boxes to summary.
    """
    bboxes = tf.expand_dims(bboxes, 0)
    image_with_box = tf.image.draw_bounding_boxes(images, bboxes)
    tf.summary.image(name, image_with_box, max_outputs=3,collections=[tf.GraphKeys.SUMMARIES])
...
def _create_losses(input_queue, create_model_fn, train_config):
  ...
  for bidx in range(len(preprocessed_images)):
    tf_summary_image(
      preprocessed_images[bidx],
      groundtruth_boxes_list[bidx],
      name='pre_image')
...
def train(create_tensor_dict_fn,
  ...
  with tf.Graph().as_default():
    ...
    ### modified #############################################################
    # in order the chose gpu in multiple gpu setting, add an offset to gpu idx
    # it's just a patch, use with your own caution
    def clone_device_with_offset(clone_index):
        ...
        # else:
          # add gpu offset term
        device += '/device:GPU:%d' % (clone_index+gpu_offset)
        return device
    deploy_config.clone_device=clone_device_with_offset
    ### end ##################################################################
