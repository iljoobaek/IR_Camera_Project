...
int main()
{
	...
	// step: Sync
	//
	FLR_BOSON_EXT_SYNC_MODE_E mode;
	result = bosonGetExtSyncMode(&mode);
printf("The external sync mode is 0X%08X \n", mode);

	result = bosonSetExtSyncMode(2);
	if(result){
		printf("Failed to set the Sync Mode with status 0x%08X, exiting.\n", result);
		Close();
		return 1;
	}
	printf("Set External Sync Mode 0x%08X \n", result);
	printf("SUCCESS\n\n");
