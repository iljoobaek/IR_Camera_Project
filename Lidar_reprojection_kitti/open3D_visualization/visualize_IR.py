...
class KittiDataset(object):
    ...
    def get_raw_calib(self, calib_velo_to_cam_path, calib_cam_to_cam_path):
        ...
        for i in range(4):
            ...
            calib['T_0%d'%i] = np.array(
                kjcalib['T_0%d'%i].split(' ')[1:], dtype=np.float32).reshape(3,1)
    ...
    def get_autoware_calib(self, calib_path, transform_mat = None):
        """Read calibrations in kitti raw dataset."""
        calib = {}
        ...
        #calib['cam_to_velo'] = np.dot(calib['cam_to_velo'], transform_mat)
        #print "This is new cam to velo matrix"
        ...
        #print calib['velo_to_image']
        print calib['velo_to_cam']
        calib['velo_to_cam'] = np.dot(calib['velo_to_cam'], transform_mat)
        print calib['velo_to_cam']
        print "This is the new transformation matrix"
        print (np.linalg.inv(calib['velo_to_cam']))
        #print np.matmul(calib['CameraMat'], calib['velo_to_cam'][:3,:])
        return calib
    ...
    def get_velo_points(self, frame_idx):
        ...
        velo_data = np.fromfile(point_file, dtype=np.float32).reshape(-1, 5)
        velo_points = velo_data[:,:3]
        reflections = velo_data[:,[3]]
        return Points(xyz = velo_points, attr = reflections)

    def get_velo_points_t(self, frame_idx):
        """Load velo points from frame_idx.
        ...
        rotation_matrix = np.array([[np.cos(theta), -np.sin(theta), 0.0],[np.sin(theta), np.cos(theta), 0.0],[0.0, 0.0, 1.0]], dtype=np.float32)
        velo_points = np.matmul(rotation_matrix, velo_points.T).T
        reflections = velo_data[:,[3]]
        return Points(xyz = velo_points, attr = reflections)
...
def test_raw(root_dir, yamlfile=None, transform_mat = None):
    kitti = KittiDataset(
    ...
    calib = kitti.get_autoware_calib(    
        os.path.join(root_dir, yamlfile), transform_mat)
    # If you are using kitti
    #calib = kitti.get_raw_calib(
    #    '../../data/kitti_data/2011_09_26/calib_velo_to_cam.txt',
    #    '../../data/kitti_data/2011_09_26/calib_cam_to_cam.txt'
    #)
    ...
    for frame_idx in range(300, kitti.num_files):
        # Get an image
        ...
        for i, img_point in enumerate(img_points.xyz):
            ...
            #    cv2.circle(point_and_img,
            #        (int(img_point[0]), int(img_point[1])), 2, color, -1, 4, 0)
        width, height = int(point_and_img.shape[1]/2), int(point_and_img.shape[0]/2)
        point_and_img = cv2.resize(point_and_img, (width, height))
...
if __name__ == '__main__':
    ...
    print mat_transform
    if len(sys.argv) > 1:
        root_dir = '../../data/data_raw/synced/' + sys.argv[1]
        yamlname = sys.argv[1] + '.yaml'
        test_raw(root_dir, yamlname, mat_transform)
    else: 
        # root_dir = '../autoware-20180928131539/'
        # root_dir = '../0216_4/'
        # root_dir = '../0217_sync/'
        #root_dir = '../../kitti_data/2011_09_26/2011_09_26_drive_0009_sync/'
        root_dir = '../../data/kitti_data/2011_09_26/2011_09_26_drive_0057_sync/'
        test_raw(root_dir)
